...
/**
* Modified abstract parent class for Pokemon species.
...
public abstract class Pokemon implements Comparable<Pokemon> {
   ...
   *@throws PokemonException if invalid 
   **/
   public Pokemon(String species, String name, 
      ...
      if (name == "") {
         throw new PokemonException("Your pokemon needs a name");
      }      
   ...
   */
   ...
   /**.
	 *Compare method
    *@return 0
    *@param othPK the Pokemon comparing to 
    */
   public int compareTo(Pokemon othPK) {
      int result = 0;    
      if (this.equals(othPK)) {
         return result;
      }
   
      if (this.getNumber() != othPK.getNumber()) {
         result = this.getNumber() - othPK.getNumber();
         return result;
      }
      
      if (this.getName() != othPK.getName()) {
         result = this.getName().compareTo(othPK.getName());
         return result;         
      }
   
      if (this.getHP() != othPK.getHP()) {
         result = this.getHP() - othPK.getHP();
         return result;
      }   
   
      if (this.getCP() != othPK.getCP()) {
         result = this.getCP() - othPK.getCP();
         return result;
      }
   
      if (this.getFastAttack() != othPK.getFastAttack()) {
         result = this.getFastAttack().compareTo(othPK.getFastAttack());
         return result;
      }
   
      if (this.getSpecialAttack() != othPK.getSpecialAttack()) {
         result = this.getSpecialAttack().compareTo(
            othPK.getSpecialAttack());
         return result;
      }
      ...
      return 0;
	   ...
	   /**.
	    *equal method 
       *@return consistancy True
       *@param othPK the Pokemon comparing to
	    */
   public boolean equals(Pokemon othPK) {
      boolean number = this.getNumber() == othPK.getNumber();
      boolean name = this.getName() == othPK.getName();
      boolean hP = this.getHP() == othPK.getHP();
      boolean cP = this.getCP() == othPK.getCP();
      boolean fastAttack = this.getFastAttack() == othPK.getFastAttack();
      boolean specialAttack = this.getSpecialAttack() 
          == othPK.getSpecialAttack();
   
      if (number && name && hP && cP && fastAttack && specialAttack) {
         return true;
      } else {
         return false;
      }
  ...
  * @throws PokemonException if newName is empty.
  */
